...
int main(int _argc, char **_argv)
 {
    ...
    FILE *storage = fopen("dorm-repository.text", "r+");
   ...
   if(strcmp(input,"---") == 0){
    ...
    if(strcmp(inputBuffer[0],"student-print-all")==0){
        ...
        if(strcmp(inputBuffer[4],"male") == 0){
            students[student_size] = cerate_student(
                inputBuffer[1],
                inputBuffer[2],
                inputBuffer[3],
                GENDER_MALE
        ...
        for(int std = 0 ; std < student_size ; ++std){
            if(strcmp(student[std].id,inputBuffer[1]) == 0){
                for(int drm = 0 ; drm < drom size; ++drm){
    ...
    if(strcmp(inputBuffer[0],"dorm-add") == 0){
        ...
        } else if(strcmp(iputBuffer[0],"dorm-print-all") == 0){
 ...
 for(int indeks = 0 ; indeks < command_counter ; ++indeks){
     if(strcmp(print_command[indeks],"student-print-all") == 0){
         students->print_student(student_print_all[indeks]);
 }
    if(strcmp(print_command[indeks],"student-print-all-detail") == 0){
        print_student_detail(student_print_all_detail[indeks]);
    }
    if(strcmp(print_command[indeks],"dorm-print-all") == 0){
        print_dorm(dorm_print_all[indeks]);
    }
 }
    fclose(storage);
    return 0;